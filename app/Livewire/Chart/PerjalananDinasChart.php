...
class PerjalananDinasChart extends Component
{
    ...
    public function render()
    {
        ...
        $years = Sppd::selectRaw('YEAR(tgl_berangkat) as year')
            ->groupBy('year')
            ->orderBy('year', 'desc')
            ->pluck('year');
        ...
        $jingan = DB::table('sppds')
            ->where('kdunit', auth()->user()->kdunit)
            ->selectRaw('MONTHNAME(MAX(tgl_berangkat)) as month, COUNT(*) as total_sppds')
            ->whereYear('tgl_berangkat', $selectedYear)
            ->groupByRaw('MONTH(tgl_berangkat)')
            ->orderByRaw('MONTH(tgl_berangkat)')
            ->get();
        ...
        return view('livewire.chart.perjalanan-dinas-chart', [
